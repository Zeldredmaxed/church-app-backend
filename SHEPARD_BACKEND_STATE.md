...
| `GET` | `/api/tenants/:tenantId/members?cursor=&limit=&missingTagIds=&tagIds=` | admin/pastor/accountant. Cursor-paginated. `?tagIds=uuid1,uuid2` returns members who have AT LEAST ONE of the listed tags (positive filter, powers communications audience targeting). `?missingTagIds=uuid1,uuid2` returns members who have NONE of the listed tags (negative filter, e.g. "newcomers not yet welcomed"). Both stack AND-style. Each capped at 20 ids. |
