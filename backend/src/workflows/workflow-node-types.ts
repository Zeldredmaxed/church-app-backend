...
export interface NodeTypeInfo {
  ...
  /**
   * Output handles the builder should render. Each entry is the exact branch
   * label to emit on the WorkflowConnection.branch field.
   * - Omit for linear nodes → builder renders a single unnamed handle (branch = 'default').
   * - Provide a fixed array for if/check-style nodes → e.g. ['true', 'false'].
   * - For dynamic branches (like switch_case where cases come from node config),
   *   leave `branches` empty and set `dynamicBranches` to the config key that
   *   holds the comma-separated case list. The builder should parse that at
   *   node-save time and render one handle per case plus a 'default' handle.
   */
  branches?: string[];
...
export const NODE_TYPE_REGISTRY: NodeTypeInfo[] = [
  ...
  // CONDITIONS
  { type: 'if_else', category: 'condition', label: 'If / Else', description: 'Generic branching: compares two values and sends the flow down the TRUE branch or the FALSE branch. Use {{context.key}} or {{trigger.key}} to reference values from earlier in the workflow.', icon: 'GitFork', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'leftValue', label: 'Left Value', type: 'text', required: true, placeholder: '{{context.attended}} or literal' }, { key: 'operator', label: 'Operator', type: 'select', options: ['equals', 'not_equals', 'contains', 'not_contains', 'greater_than', 'less_than', 'is_empty', 'is_not_empty', 'is_true', 'is_false'], required: true }, { key: 'rightValue', label: 'Right Value', type: 'text', required: false, placeholder: 'What to compare against (blank for is_empty / is_true / is_false)' }] },
  { type: 'switch_case', category: 'condition', label: 'Switch / Case', description: 'Multi-way branching: routes the flow to one of several named branches based on a value. Every branch you draw must match a case name (or "default" for the fall-through).', icon: 'Share2', color: 'amber', dynamicBranches: 'cases', configFields: [{ key: 'value', label: 'Value to match', type: 'text', required: true, placeholder: '{{context.tier}}' }, { key: 'cases', label: 'Cases (comma-separated)', type: 'text', required: true, placeholder: 'new_visitor,returning,member' }] },
  { type: 'check_member_data', category: 'condition', label: 'Check Member', description: 'Branch on member data', icon: 'User', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'field', label: 'Field', type: 'select', options: ['role', 'has_phone', 'has_email', 'full_name'], required: true }, { key: 'operator', label: 'Operator', type: 'select', options: ['equals', 'not_equals', 'exists', 'not_exists'], required: true }, { key: 'value', label: 'Value', type: 'text', required: false }] },
  { type: 'check_date', category: 'condition', label: 'Check Date', description: 'Branch on date comparison', icon: 'Calendar', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'dateField', label: 'Date Field', type: 'select', options: ['today', 'day_of_week'], required: true }, { key: 'operator', label: 'Operator', type: 'select', options: ['equals', 'before', 'after'], required: true }, { key: 'value', label: 'Value', type: 'text', required: true }] },
  { type: 'check_tag', category: 'condition', label: 'Has Tag?', description: 'Check if member has a tag', icon: 'Tag', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'tagId', label: 'Tag', type: 'tag', required: true }] },
  { type: 'check_attendance', category: 'condition', label: 'Attendance Check', description: 'Check attendance frequency', icon: 'CheckCircle', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'minCount', label: 'Min check-ins', type: 'number', required: true }, { key: 'days', label: 'In last N days', type: 'number', required: true }] },
  { type: 'check_giving', category: 'condition', label: 'Giving Check', description: 'Check giving amount', icon: 'DollarSign', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'minAmount', label: 'Min amount', type: 'number', required: true }, { key: 'days', label: 'In last N days', type: 'number', required: true }] },
  { type: 'check_group_membership', category: 'condition', label: 'In Group?', description: 'Check group membership', icon: 'Users', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'groupId', label: 'Group', type: 'group', required: true }] },
  { type: 'check_engagement', category: 'condition', label: 'Engagement Level', description: 'Check engagement score', icon: 'Activity', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'level', label: 'Min Level', type: 'select', options: ['inactive', 'low', 'medium', 'high'], required: true }] },
  { type: 'check_journey_stage', category: 'condition', label: 'Journey Stage', description: 'Check spiritual milestone', icon: 'Milestone', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'milestone', label: 'Milestone', type: 'select', options: ['attended_members_class', 'is_baptized', 'salvation_date', 'discipleship_foundations', 'discipleship_growth', 'discipleship_leadership', 'discipleship_completed'], required: true }] },
  { type: 'always_true', category: 'condition', label: 'Always True', description: 'Always takes the true branch', icon: 'Check', color: 'amber', branches: ['true'], configFields: [] },
  { type: 'check_badge', category: 'condition', label: 'Has Badge?', description: 'Check if member has a specific badge', icon: 'Award', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'badgeId', label: 'Badge', type: 'badge', required: true }] },
  { type: 'weather_check', category: 'condition', label: 'Weather Check', description: 'Branch based on current weather (temperature, conditions). Uses church location.', icon: 'CloudSun', color: 'amber', branches: ['true', 'false'], configFields: [{ key: 'condition', label: 'Condition', type: 'select', options: ['temp_above', 'temp_below', 'temp_between', 'is_raining', 'is_snowing', 'is_stormy'], required: true }, { key: 'tempThreshold', label: 'Temperature (F)', type: 'number', required: false, placeholder: '95' }, { key: 'tempMin', label: 'Min Temp (F) — for between', type: 'number', required: false }, { key: 'tempMax', label: 'Max Temp (F) — for between', type: 'number', required: false }, { key: 'latitude', label: 'Latitude (leave blank for church location)', type: 'number', required: false }, { key: 'longitude', label: 'Longitude (leave blank for church location)', type: 'number', required: false }] },
